...
CREATE OR REPLACE PROCEDURE add_customer (_first_name VARCHAR, _last_name VARCHAR, _birth_date DATE, _email VARCHAR)
RETURNS void
AS $$
...
CREATE OR REPLACE PROCEDURE add_services (_services_name VARCHAR)
RETURNS void
AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE add_car (_color VARCHAR, _make VARCHAR, _model VARCHAR, __year VARCHAR)
RETURNS void
AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE add_parts (_part_name VARCHAR, _part_cost VARCHAR, _part_quantity INTEGER)
RETURNS void
AS $$
BEGIN
	INSERT INTO car (part_name, part_cost, part_quantity)
...
--add sale invoice
CREATE OR REPLACE PROCEDURE add_sale_invoice (_date_sold DATE, _amount NUMERIC(10,2), _msrp NUMERIC(10,2), _customer_id INTEGER, _vin_id INTEGER)
RETURNS void
AS $$
BEGIN
	INSERT INTO car (date_sold, amount, msrp, customer_id, vin_id)
	VALUES (_date_sold, _amount, _msrp, _customer_id, _vin_id);
END;
$$
LANGUAGE plpgsql

SELECT add_sale_invoice(1/3/2020,  16495,  17750, 3, 3);

--add service invoice
CREATE OR REPLACE PROCEDURE add_service_invoice (_date_serviced DATE, _customer_id INTEGER, _part_id INTEGER, _vin_id INTEGER)
RETURNS void
AS $$
BEGIN
...
SELECT add_service_invoice(1/4/2020,  4, 2, 4);

--add salesperson invoice
CREATE OR REPLACE PROCEDURE add_salesperson_invoice (_salesperson_id INTEGER, _sale_invoice_id INTEGER)
RETURNS void
AS $$
BEGIN
...
--add labor
CREATE OR REPLACE PROCEDURE add_labor (_service_invoice_id INTEGER, _mechanic_id INTEGER, _services_id INTEGER, _labor_cost NUMERIC(10,2))
RETURNS void
AS $$
BEGIN
